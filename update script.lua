-- [[ SAN DIEGO - SILENT AIM + ESP (KHÔNG AIMBOT) ]]
-- [[ Silent Aim: chỉ xoay hướng đạn, không xoay camera ]]
-- [[ ESP: Box, Health, Lines, Skeleton ]]

if _G.SD_Silent_Cleanup then pcall(_G.SD_Silent_Cleanup) end
...
local function wipeOldUI(folder)
	...
	for _, child in ipairs(folder:GetChildren()) do
		if child.Name:find("SD_Silent_UI") then child:Destroy() end
	end
end
...
screenGui.Name = "SD_Silent_UI_" .. math.random(1000, 9999)
...
local fovStroke = Instance.new("UIStroke") fovStroke.Color = Color3.fromRGB(255, 0, 255) fovStroke.Thickness = 1.5 fovStroke.Parent = fovFrame
...
scrollContainer.ScrollBarImageColor3 = Color3.fromRGB(255, 0, 255)
...
createToggle("SilentAim", "Silent Aim", true)
...
-- [[ SILENT AIM: SỬA HƯỚNG ĐẠN BẰNG CÁCH XOAY ROOTPART ]]
local function ApplySilentAim(targetPart)
	...
	local gunPos = rootPart.Position
	if tool then
		local handle = tool:FindFirstChild("Handle")
		if handle then gunPos = handle.Position end
	end
	
	local direction = (targetPart.Position - gunPos).Unit
	-- Xoay nhân vật theo hướng mục tiêu (ảnh hưởng đến hướng đạn)
...
local function getDrawings(player)
	if not DrawObjects[player] then
		local boxLines = {
			Top = createLine(Color3.fromRGB(0, 255, 255)),
			Bottom = createLine(Color3.fromRGB(0, 255, 255)),
			Left = createLine(Color3.fromRGB(0, 255, 255)),
			Right = createLine(Color3.fromRGB(0, 255, 255))
		}
...
RunService.RenderStepped:Connect(function()
	-- Silent Aim (xoay nhân vật, không xoay camera)
...
print("[[ SILENT AIM + ESP ĐÃ TẢI ]]")
print("[[ Silent Aim: xoay nhân vật (hướng đạn) mà không xoay Camera ]]")
