-- MOBILE ESP MENU - FIX UI OVERLAP & POSITION
...
----------------------------------------------------
-- 1. DỌN SẠCH UI CŨ & TẠO MENU MỚI CHUẨN ĐẸP
----------------------------------------------------
local playerGui = LocalPlayer:WaitForChild("PlayerGui")

-- Xóa tất cả các Menu cũ từng tạo để tránh bị đè/chồng UI
for _, oldUI in ipairs(playerGui:GetChildren()) do
	if oldUI.Name == "Delta_ESP_UI" or oldUI.Name == "ESP_MobileMenu" or oldUI.Name == "ESP_Mobile_Fix" then
		oldUI:Destroy()
	end
end
...
-- Nút Bật/Tắt Menu tròn (Đặt ở góc trái gọn gàng)
...
-- Bảng Menu (Đã chỉnh tọa độ X tránh bị thụt vào mép màn hình)
local menuFrame = Instance.new("Frame")
menuFrame.Name = "Menu"
menuFrame.Size = UDim2.new(0, 190, 0, 150)
menuFrame.Position = UDim2.new(0.12, 0, 0.28, 0) -- Dịch sang phải để không bị che
...
-- Container tự căn chỉnh vị trí nút
...
----------------------------------------------------
-- 3. KHỞI TẠO NÚT BẤM & QUÉT PLAYER
----------------------------------------------------
